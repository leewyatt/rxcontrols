...
import io.github.leewyatt.rxcontrols.samples.demo.RXRangeSliderDemo;
...
/**
 * Showcase application for {@link RXRangeSlider}.
 *
 * <p>Exposes every configurable property — the dual value model, the minimum gap
 * and draggable band, indicator display and position, orientation, size variant,
 * the tick scale, and the Material feedback. For a minimal example see
 * {@link RXRangeSliderDemo}.</p>
 */
public class RXRangeSliderShowcase extends RXShowcaseApplication {
    ...
    private RXRangeSlider slider;

    // ==================== Showcase wiring ====================

    @Override
    protected String title() {
        return "RXRangeSlider";
    }

    @Override
    protected String subtitle() {
        return "Material two-thumb range slider";
    }

    @Override
    protected String windowTitle() {
        return "RXRangeSlider Showcase";
    }

    @Override
    protected String stylesheetPath() {
        return getClass().getResource("rx-range-slider-showcase.css").toExternalForm();
    }

    @Override
    protected Node createPreview() {
        slider = new RXRangeSlider(0.0, 100.0, 30.0, 70.0);
    ...
    private Node buildModelGrid() {
        ...
        lowLabel.getStyleClass().add("value-label");
        lowLabel.textProperty().bind(Bindings.format("%.1f", slider.lowValueProperty()));
        Label highLabel = new Label();
        highLabel.getStyleClass().add("value-label");
        highLabel.textProperty().bind(Bindings.format("%.1f", slider.highValueProperty()));
    ...
    private Node buildRangeGrid() {
        Slider minGap = createSlider(0.0, 40.0, slider.getMinGap());
        slider.minGapProperty().bind(minGap.valueProperty());

        CheckBox draggable = new CheckBox("Range draggable (drag the band)");
        draggable.setSelected(slider.isRangeDraggable());
        slider.rangeDraggableProperty().bind(draggable.selectedProperty());

        return createGrid(
                row("Min gap", minGap, createValueLabel(minGap, "%.0f")),
                row(draggable));
    }
