...
/**
 * Controller for the embedded command-menu sampler page: an anchored account
 * menu (icons + separator + shortcuts + danger) and a right-click context menu
 * installed on a card, mirroring the standalone {@code RXMenuButtonDemo}.
 */
public class PaneMenuController {
    ...
    void initialize() {
        ...
        content.getChildren().setAll(buildAccountMenu(status), buildContextCard(status), status);
    }
